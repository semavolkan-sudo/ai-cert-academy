...
function TurkishCharTest(props) {
    ...
    return (
      <button onClick={function() { setOpen(true); }}
        style={{ position:"fixed", bottom:16, right:16, zIndex:9999,
          width:44, height:44, borderRadius:"50%", border:"1px solid "+CARD_BORDER2,
          background:"linear-gradient(135deg,"+GOLD+","+GOLD2+")",
          color:"#0d0d1f", fontSize:18, fontWeight:700, cursor:"pointer",
          display:"flex", alignItems:"center", justifyContent:"center",
          boxShadow:SHADOW_GOLD, transition:"all 0.2s ease" }}
  ...
  return (
    <div style={{ position:"fixed", bottom:16, right:16, zIndex:9999,
      ...
      background:"linear-gradient(145deg,"+BG2+","+BG+")",
      border:"1px solid "+CARD_BORDER2, borderRadius:RADIUS,
      boxShadow:SHADOW, padding:20, fontFamily:FONT, color:TEXT }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16 }}>
        <div style={{ fontSize:15, fontWeight:700, color:GOLD }}>Türkçe Karakter Testi</div>
        <button onClick={function() { setOpen(false); }}
          style={{ background:"none", border:"none", color:TEXT2, fontSize:18, cursor:"pointer" }}>×</button>
      </div>
      {chars.map(function(row, ri) {
        return (
          <div key={ri} style={{ marginBottom:14 }}>
            <div style={{ fontSize:11, color:TEXT2, marginBottom:6, textTransform:"uppercase", letterSpacing:1 }}>{row.label}</div>
            <div style={{ display:"flex", flexWrap:"wrap", gap:8 }}>
              {row.chars.map(function(ch, ci) {
                return (
                  <div key={ci}
                    style={{ minWidth:36, height:36, display:"flex", alignItems:"center", justifyContent:"center",
                      ...
                      padding: row.chars[0].length > 1 ? "0 10px" : 0 }}>
                    {ch}
                  </div>
                );
              })}
      ...
      <div style={{ marginTop:10, paddingTop:10, borderTop:"1px solid "+CARD_BORDER, fontSize:11, color:TEXT2, lineHeight:1.5 }}>
        Font: <span style={{ color:GOLD }}>{FONT.replace(/'/g,"")}</span><br/>
        Mono: <span style={{ color:GOLD }}>{FONT_MONO.replace(/'/g,"")}</span>
      </div>
    </div>
  );
}
