      ...
      <div style={{ minHeight:"100vh", background:BG, color:"#fff", fontFamily:FONT, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}>
        <div style={{ width:"100%", maxWidth:420 }}>
          ...
          <div style={{ background:CARD_BG, border:"1px solid "+CARD_BORDER, borderRadius:16, padding:28 }}>
            ...
            <p style={{ color:"#9999b8", fontSize:13, marginBottom:20 }}>E-posta adresinize sıfırlama bağlantısı göndereceğiz.</p>
            {!resetSent ? (
              <div>
                <label style={{ display:"block", color:"#888899", fontSize:12, marginBottom:5 }}>E-posta Adresi</label>
                <input
                  value={resetEmail}
                  onChange={function(e) { setResetEmail(e.target.value); setResetErr(""); }}
                  placeholder="ornek@ornek.com"
                  type="email"
                  style={{ width:"100%", background:"rgba(255,255,255,0.05)", border:"1px solid rgba(255,255,255,0.12)", borderRadius:12, padding:"13px 16px", color:"#fff", fontSize:15, outline:"none", marginBottom:8, boxSizing:"border-box" }}
                />
                ...
                <button
                  ...
                  style={{ width:"100%", background:"linear-gradient(135deg,#d4a853,#f0c060)", color:"#08080f", border:"none", borderRadius:12, padding:"14px", fontSize:15, fontWeight:700, cursor: resetLoading ? "not-allowed" : "pointer", marginBottom:12 }}>
                  {resetLoading ? "Gönderiliyor..." : "Sıfırlama Bağlantısı Gönder"}
                </button>
                <button
                  onClick={function() { setShowReset(false); setResetErr(""); setResetEmail(""); }}
                  style={{ width:"100%", background:"transparent", border:"1px solid rgba(255,255,255,0.12)", borderRadius:12, padding:"12px", color:"#9999b8", fontSize:13, cursor:"pointer" }}>
                  Geri Dön
                </button>
              </div>
            ) : (
              <div style={{ textAlign:"center" }}>
                ...
                <p style={{ color:"#bbbbcc", fontSize:13, marginBottom:14, lineHeight:1.5 }}>
                  <strong style={{ color:GOLD }}>{resetEmail}</strong> adresine sıfırlama bağlantısı gönderildi. Gelen kutunuzu kontrol edin.
                </p>
                <p style={{ color:"#777799", fontSize:12, marginBottom:18 }}>E-posta gelmedi mi? Spam klasörünü kontrol edin.</p>
                <button
                  onClick={function() { setShowReset(false); setResetSent(false); setResetEmail(""); setResetErr(""); }}
                  style={{ width:"100%", background:"linear-gradient(135deg,#d4a853,#f0c060)", color:"#08080f", border:"none", borderRadius:12, padding:"13px", fontSize:14, fontWeight:700, cursor:"pointer" }}>
                  Giriş Ekranına Dön
                </button>
              </div>
            )}
