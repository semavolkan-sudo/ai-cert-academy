...
var PLANS = [
  ...
  { name: "Pro",      price: 79,  color: "#d4a853", popular: true,
    features: ["Tüm araçlara tam erişim", "Tüm dersler", "XP ve Streak sistemi", "AI Mentor (4 seans)", "Bonus içerikler", "Premium sertifika"] },
...
var COURSES = [
  ...
  { day:17, tool:"NotebookLM",         icon:"\uD83D\uDCD3", color:"#7c3aed", desc:"Google NotebookLM ile dokümanlarınızı yapay zekâyla analiz edin" },
  { day:18, tool:"Microsoft 365 Copilot", icon:"\uD83D\uDCCA", color:"#0f6cbd", desc:"Word, Excel ve Teams içinde Copilot ile üretkenlik" },
  ...
  function send() {
    ...
    fetch(PROXY_URL, {
      ...
      body: JSON.stringify({ model:"claude-sonnet-4-6", max_tokens:800,
        system:"Sen AI Certification Academy'nin eğitim mentorusun. Öğrenci: "+user.name+". Plan: "+(user.plan?user.plan.name:"")+". Seviye: "+lvl.name+" ("+(user.xp||0)+" XP). Görevin: platformdaki AI araçlarını öğrenmesine yardım etmek. Türkçe, samimi ve kısa yanıtlar ver. Kurallar: Bilmediğin konuda uydurma, dürüstçe söyle. Kazanç garantisi veya abartılı vaat verme. Eğitim dışı konularda (sağlık, hukuk, yatırım tavsiyesi) nazikçe konuya dön. Kişisel veri isteme.",
        messages:history })
    }).then(function(r) {
  ...
  return (
    <div style={{ minHeight:"100vh", background:BG, color:TEXT, fontFamily:FONT, backgroundImage:"radial-gradient(circle at 1px 1px, rgba(255,255,255,0.04) 1px, transparent 0)", backgroundSize:"32px 32px" }}>
      ...
      <div style={{ position:"relative", textAlign:"center", padding:"72px 20px 80px", background:"radial-gradient(ellipse at 50% 0%,rgba(124,92,252,0.18) 0%,rgba(201,168,76,0.10) 30%,transparent 70%)" }}>
        <div style={{ display:"inline-block", background:"rgba(201,168,76,0.10)", border:"1px solid rgba(201,168,76,0.4)", borderRadius:100, padding:"7px 18px", fontSize:12, color:GOLD2, marginBottom:28, fontFamily:FONT_MONO }}>{"28 AI aracı · Kendi hızında · Günde 15 dk yeter"}</div>
        ...
        <p style={{ fontSize:16, color:TEXT2, maxWidth:460, margin:"0 auto 40px", lineHeight:1.65 }}>28 AI aracını kendi hızında, günde 15 dakikayla öğren.<br/><span style={{ color:GOLD2 }}>AI sertifikanı kazan, kariyerini dönüştür.</span></p>
    ...
    <div style={{ background:BG, color:TEXT, fontFamily:FONT, padding:"72px 20px 88px", borderTop:"1px solid rgba(255,255,255,0.06)" }}>
      <div style={{ maxWidth:980, margin:"0 auto", display:"flex", gap:48, flexWrap:"wrap", alignItems:"flex-start" }}>
        <div style={{ flex:"1 1 380px", minWidth:300 }}>
          ...
          <div style={{ fontSize:32, fontWeight:800, lineHeight:1.25, marginBottom:16 }}>Ekibinize AI yetkinliğini<br/>uygulayarak kazandırın</div>
          <p style={{ color:TEXT2, fontSize:15, lineHeight:1.7, marginBottom:28 }}>Eğitmen maliyeti yok, kurulum yok. Çalışanlarınız günde 15 dakikayla 28 AI aracını kendi hızlarında, uygulayarak öğrenir; siz ilerlemeyi tek panelden izlersiniz.</p>
              ...
              {COURSES.map(function(c) {
                ...
                return (
                  <div key={c.day} onClick={function() {
                    if (isLocked) { setUpgrade("Modül " + c.day + ": " + c.tool); return; }
                    props.onLesson(c);
                  }}
...
var TOOL_USECASES = {
  ...
  "NotebookLM": { tagline: "Google'ın doküman analiz asistanı", usecases: ["📚 Şirket dokümanlarını yükleyip soru sor","🎙️ Dokümanlardan sesli özet (Audio Overview) üret","🔍 Kaynak göstererek güvenilir yanıtlar al","🗂️ Araştırma notlarını tek yerde yönet"] },

  "Microsoft 365 Copilot": { tagline: "Office uygulamalarının içindeki AI asistan", usecases: ["📝 Word'de taslak ve özet çıkar","📊 Excel'de veriyi analiz edip formül yaz","📧 Outlook'ta e-posta yanıtlarını hızlandır","👥 Teams toplantı özetleri al"] },
  ...
  return (
    <div style={{ minHeight:"100vh", background:BG, color:"#fff", fontFamily:FONT }}>
      <div style={{ borderBottom:"1px solid rgba(255,255,255,0.06)", padding:"12px 24px", display:"flex", alignItems:"center", gap:12 }}>
        ...
        <span style={{ fontWeight:700, fontSize:14 }}>{"Modül " + lesson.day + ": " + lesson.tool}</span>
